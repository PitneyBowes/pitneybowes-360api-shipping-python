...
class ShipmentERRCoversheet(BaseModel):
    """
    ...
    from_address: FromAddress = Field(alias="fromAddress")
    to_address: ToAddress = Field(alias="toAddress")
    parcel: Parcel
    carrier_account_id: Optional[StrictStr] = Field(default=None, description="A unique identifier associated with the user's registered USPS account which is used by client users while shipment process.", alias="carrierAccountId")
    parcel_type: Optional[StrictStr] = Field(default=None, description=">-Packaging type varies as per USPS selected services, e.g., LTR, LGENV.", alias="parcelType")
    service_id: Optional[StrictStr] = Field(default=None, description=">-A unique identifier given to the carrier-specific service. ERR supports two services: First Class Mail (FCM) and Priority Mail (PM).", alias="serviceId")
    date_of_shipment: Optional[StrictStr] = Field(default=None, description="The date when shipment gets created.", alias="dateOfShipment")
    special_services: Optional[List[SpecialServiceERRInner]] = Field(default=None, alias="specialServices")
    shipment_options: Optional[ShipmentOptionsERR] = Field(default=None, alias="shipmentOptions")
    metadata: Optional[List[ShipmentERRCoversheetMetadataInner]] = Field(default=None, description="Additional metadata that needs to be stored for this shipment, can be added here. For now, `costAccountName` is supported.")
    __properties: ClassVar[List[str]] = ["externalId", "fromAddress", "toAddress", "parcel", "carrierAccountId", "parcelType", "serviceId", "dateOfShipment", "specialServices", "shipmentOptions", "metadata"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "shipmentOptions": ShipmentOptionsERR.from_dict(obj["shipmentOptions"]) if obj.get("shipmentOptions") is not None else None,
            "metadata": [ShipmentERRCoversheetMetadataInner.from_dict(_item) for _item in obj["metadata"]] if obj.get("metadata") is not None else None
        })
        return _obj
