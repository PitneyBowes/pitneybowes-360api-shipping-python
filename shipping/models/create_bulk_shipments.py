...
class CreateBulkShipments(BaseModel):
    """
    ...
    name: StrictStr
    carrier_account_id: StrictStr = Field(description="A unique identifier associated with the Carrier account used by client users during shipment process. Default CarrierAccountID for this batch will be user's registered carrier account. User can override this value by defining it at shipment level.", alias="carrierAccountId")
    parcel_type: StrictStr = Field(description="Parcel Type is required for creating a shipment while rating a parcel, which varies as per Carrier selection. Different carriers have different ParcelTypes, e.g., FRPKG, LGENV, TUBE, and PKG. User can override this value by defining it at Shipment level.", alias="parcelType")
    service_id: StrictStr = Field(description="A unique identifier given to the carrier-specific service. It varies with carrier selection. This is required for creating a shipment, while it is optional for rating a parcel. User can override this value by defining it at Shipment level.", alias="serviceId")
    special_services: Optional[List[SpecialService]] = Field(default=None, description="It provides a carrier-service based special or extra service, which also varies based on services and parcel types. User can override this value by defining it at shipment level.", alias="specialServices")
